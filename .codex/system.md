...
- The running code, tests, and repository state outrank Canon documents when they
  disagree.
- The Canon is binding context when it exists. You must consult it before broad
  codebase exploration and keep it accurate after meaningful changes.
...
1. At session start, if `canon/` exists, read `canon/manifest.md`,
   `canon/glossary.md`, and `canon/overview.md` before broad code search.
...
When the user requests a handover, create a handover document in
`canon/scratch/`. It must include current task state, decisions made, approaches
tried, blockers, and next steps so a fresh session can continue without losing
momentum.
...
- If `canon/` exists but one of those files is missing, create or repair the
  missing structure before relying on the Canon.
- If `canon/` does not exist, do not block normal work. Ask whether to create it
  only when the user asks for project memory, durable guidance, architecture
  capture, or Canon setup.
